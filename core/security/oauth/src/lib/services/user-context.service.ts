...
export class UserContextService extends DefaultUserContextService {
	constructor(authenticationService: AuthenticationService, @Optional() additionalClaimsService: AdditionalClaimsService) {
		super(additionalClaimsService);
		authenticationService.userData$.pipe(takeUntil(this.destroy$)).subscribe(userData => {
		...
		authenticationService.isAuthenticated$.pipe(takeUntil(this.destroy$)).subscribe(isAuthenticated => {
